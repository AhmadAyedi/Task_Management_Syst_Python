...
def admin_interface():
    ...
    def update_user():
        selected_user = user_listbox.curselection()
        if selected_user:
            old_username = user_listbox.get(selected_user)
            ...
            if old_username != new_username and os.path.exists(f"{new_username}.txt"):
                messagebox.showerror("Error", "A user with this username already exists!")
                return

            os.rename(f"{old_username}.txt", f"{new_username}.txt")
            with open(f"{new_username}.txt", 'w') as file:
    ...
    tk.Label(admin_win, text="New/Updated Username:").pack(pady=5)
    new_user_entry = tk.Entry(admin_win)
    new_user_entry.pack()

    tk.Label(admin_win, text="New/Updated Password:").pack(pady=5)
    new_pass_entry = tk.Entry(admin_win)
    new_pass_entry.pack()

    tk.Button(admin_win, text="Create User", command=create_user, bg="#4CAF50", fg="white").pack(pady=5)
    tk.Button(admin_win, text="Update User", command=update_user, bg="#FFC107", fg="black").pack(pady=5)
    tk.Button(admin_win, text="Delete User", command=delete_user, bg="#F44336", fg="white").pack(pady=5)
    tk.Button(admin_win, text="Open User Task Manager", command=open_user_tasks, bg="#2196F3", fg="white").pack(pady=5)
def main_window():
