...
def open_task_manager(username):
    ...
    modify_button = tk.Button(task_win, text="Modify Task", command=lambda: modify_task(username), bg="#FFC107", fg="black")
    modify_button.grid(row=2, column=2)
    tk.Button(task_win, text="Delete Task", command=lambda: delete_task(username), bg="#F44336", fg="white").grid(row=3, column=2)
